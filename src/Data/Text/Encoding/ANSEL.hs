...
decodeANSEL bs =
  T.pack . concat
    . evalState (whileM (gets (not . null)) decodeANSELChar)
    $ B.unpack bs

decodeANSELChar :: State [Word8] String
decodeANSELChar = do
  (dias', rest) <- gets (span isDiacritic)
  let dias = filter (/= '\xFFFD') . fmap (combiningTable !) $ dias'
  case rest of
    [] -> put [] $> if null dias then "" else "\xFFFD"
    r : rs ->
      put rs
        $> let c = encode r
            in if c == '\xFFFD' then "\xFFFD" else c : dias
  where
    encode r = if isAscii r then toEnum (fromIntegral r) else composedTable ! r
